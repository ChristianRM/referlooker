...
def evaluate_candidate(profile_data: dict, vacancy_text: str, config: dict) -> dict:
    ...
    # 2. Proceed to LLM evaluation for all candidates
    ...
    otw_desc = "Active Job Seeker (Confirmed via LinkedIn Badge/Card - " + otw_source + ")" if is_otw_detected else ("Active Job Seeker (Detected in profile text)" if has_text_signal else "Passive Candidate / Not explicitly indicated")
    profile_summary = f"""
URL: {profile_data.get('url')}
Name: {profile_data.get('name')}
Headline: {profile_data.get('headline')}
Open to Work Status: {otw_desc}
"""
...
{{
  ...
  "suggested_message": "[A concise, highly personalized and friendly 3-4 sentence outreach message in Spanish or English matching the vacancy's language, mentioning candidate's specific background and asking if they are open to chat]"
}}

Instructions & Rules:
1. **Open to Work Status (Informational Badge)**:
   * Set "open_to_work" to true if the candidate has active job search indicators (#OpenToWork badge, looking for work, disponible, etc.).
   * Set "open_to_work" to false if they are a passive candidate or not explicitly searching.
   * NOTE: Do NOT penalize the candidate's technical or experience scores if open_to_work is false. Treat them as a valid candidate and evaluate their capabilities objectively.

2. **Scoring Rubric (Applies to all candidates)**:
   ...
   * **auxiliary_score** (0-20): Fit for databases, cloud platforms, DevOps, soft skills, and other auxiliary tools.
   * **match_score**: MUST be equal to technical_score + experience_score + auxiliary_score (0-100).

3. **Geographic Location Check**:
   If the job description specifies a mandatory location/country (e.g. USA, United States, Mexico), check if the candidate lives there. If they live in a completely different country, set the match_score and all sub-scores to 0.
...
"""
    ...
    try:
        ...
        detected_otw = bool(has_active_signal or data.get("open_to_work", False))
        return {
...
def generate_outreach_message_llm(candidate: dict, vacancy_text: str = "", config: dict = None) -> str:
    """
    Generates or customizes an outreach message for a specific candidate using LLM.
    """
    config = config or {}
    model_name = config.get("ollama", {}).get("model", "llama3.1:8b")
    ollama_host = config.get("ollama", {}).get("host", "http://localhost:11434")
    ...
    first_name = name.split()[0] if name and name != "Unknown" else "Hola"
    vacancy = candidate.get("vacancy", "Posición Abierta").replace(".txt", "")
    headline = candidate.get("headline", "")
    summary = candidate.get("evaluation_summary", "")
    about = (candidate.get("about") or "")[:500]
    
    prompt = f"""
You are a top-performing technical recruiter. Write a short, personalized, professional and warm LinkedIn outreach message (3-4 sentences max) to {first_name}.
Position: {vacancy}
Candidate Headline: {headline}
Key Candidate Strengths: {summary}
Candidate Summary: {about}

Requirements:
- Sound human, engaging, not like a generic bot.
- Mention 1 specific strength or relevance of their profile.
- Clear, low-pressure Call to Action (e.g. asking if they'd be open to a quick 10-minute chat this week).
- Write in Spanish if the context/name is Latin American / Mexican, or English if standard international.

Respond ONLY with the message text. Do not add quotes, subject line or markdown formatting.
"""
    try:
        client = ollama.Client(host=ollama_host)
        response = client.generate(model=model_name, prompt=prompt, options={"temperature": 0.4})
        return response.get("response", "").strip()
    except Exception as e:
        print(f"[Warning] Failed to generate outreach message via LLM: {e}")
        from database import generate_default_outreach_message
        return generate_default_outreach_message(name, vacancy, summary)
